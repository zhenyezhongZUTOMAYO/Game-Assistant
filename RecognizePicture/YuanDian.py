    ...
    def trackingYuanDian(self, lock):
        ...
        while True:
            ...
            if not self.one:
                ...
                # print("正在操作")
                self.signal[2] = 0
            if (abs(self.rec.x - center_x) > convert_coordinates(900, 0, (3840, 2160), rec.resolutionRatio)[0] or abs(
                    self.rec.y - center_y) > convert_coordinates(0, 580, (3840, 2160), rec.resolutionRatio)[
                    1]) and not avoid:
                stick += 1
